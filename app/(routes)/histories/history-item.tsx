...
import RatingBadge from '@/app/_components/ui/rating-badge'
...
function HistoryItemAction({ history }: { history: ArrayElement<Histories> }) {
  const { id, rating, review, expired } = history

  if (rating !== null && review !== null)
    return <HistoryItemRatedAndReviewed history={history} />

  if (rating !== null && expired)
    return <HistoryItemRatedButReviewSessionExpires />

  return (
    <Link href={`/experiences/${id}`}>
      {rating === null && <HistoryItemNotRated />}
      {rating !== null && <HistoryItemRatedButNotReviewed />}
    </Link>
  )
}
    ...
    <Drawer>
      ...
      <DrawerContent>
        ...
        <div className="mx-4 my-2 rounded-md border border-muted px-4 py-3">
          ...
          <p className="text-sm font-light">{history.review}</p>
        </div>
...
function HistoryItemRatedButNotReviewed() {
  return (
    <div className="rounded-md border p-4">
      <div className="flex items-center">
        <RatingBadge />
